...
def mark_attendance_api(request):
    if request.method == 'POST':
        try:
            ...
            # Create image
            stdNumb = student.user.username # getting student number
            file_name = f"{stdNumb}_unknown.png"
            file_path = f"./webapp/unknown/{file_name}"

            with open(file_path, 'wb') as f:
                f.write(image_data)

            # Use face recognition to confirm identity
            try:
                iden = checkIdentity(stdNumb, file_path)
            except IndexError:
                return JsonResponse({'error':"We weren't able to locate any faces in your submited image pleas try again(remove glases and make sure you're in good lighting)" }, status=404)

            # Only happens if identity is confirmed
            if iden == True:
                ...
                return JsonResponse({'error': "Submited face doesn't match identity on record"}, status=404)
